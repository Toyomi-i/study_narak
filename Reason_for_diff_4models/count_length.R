...
get_counts_leng <- function(data) {
  counts[[1]] <- apply(data, 2, mean)
  counts[[2]] <- apply(data, 2, sd)
  counts[[3]] <- apply(data, 2, max)
  counts[[4]] <- apply(data, 2, min)
  return(counts)
}
## sub func. up to here ##
...
for (group_num in 1:length(groups)) {
    counts <- get_counts_leng(groups[[group_num]][s_idx:e_idx])
